...
const userTypeSelector = document.getElementById("filter");
...
function buildAdminUsersFetchURI(caller, value) {
    let uri = "/api/admin/users?";

    var search, userType, sort, order, page;

    search = caller === "search" ? value : searchInput.value;
    userType = caller === "userType" ? value : userTypeSelector.value;
    ...
    if (userType && userType !== "all") uri += `q-user_type=${encodeURI(userType)}&`;
...
// User type Filter Handler
userTypeSelector.addEventListener("change", function() {
    const uri = buildAdminUsersFetchURI("userType", this.value);
