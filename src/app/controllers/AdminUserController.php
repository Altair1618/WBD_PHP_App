...
class AdminUserController
{
  ...
  public function getManyUsers($params)
  {
    ...
    $params['q-sort_param'] = ['id', 'nama', 'username', 'created_at', 'updated_at'][$params['q-sort_param'] ?? 0] ?? 'id';
    $params['q-sord_order'] = ['asc', 'desc'][$params['q-sord_order'] ?? 0] ?? 'asc';
    $params['page'] = $params['page'] ?? 1;

    $params['q-user_type'] = $params['q-user_type'] ?? 0;
    if ($params['q-user_type'] == 0) {
      $params['q-user_type'] = null;
    }

    require_once MODELS_DIR . 'Pengguna.php';
    $user_repo = new PenggunaRepository();
    $params['users'] = $user_repo->getPenggunaFiltered($params['q-search'], $params['q-user_type'], $params['q-sort_param'], $params['q-sord_order'], $params['page'], ITEMS_PER_PAGE);
...
?>
    <div class="table-container" id="table-container">
        ...
        <?php foreach ($users as $user) : ?>
          <?php if (!is_admin($user)) : ?>
            <div class="cell cell-value img-wrapper">
              <img src="<?= get_img_src($user) ?>" class="cell-img">
            </div>
          <?php endif ?>
        <?php endforeach ?>
      </div>
      <div class="table-column" id="column-name">
        ...
        <?php foreach ($users as $user) : ?>
          <?php if (!is_admin($user)) : ?>
            <p class="cell cell-value"><?= $user['nama'] ?></p>
          <?php endif ?>
        <?php endforeach ?>
      </div>
      <div class="table-column" id="column-username">
        ...
        <?php foreach ($users as $user) : ?>
          <?php if (!is_admin($user)) : ?>
            <p class="cell cell-value"><?= $user['username'] ?></p>
          <?php endif ?>
        <?php endforeach ?>
      </div>
      <div class="table-column" id="column-email">
        ...
        <?php foreach ($users as $user) : ?>
          <?php if (!is_admin($user)) : ?>
            <p class="cell cell-value"><?= $user['email'] ?></p>
          <?php endif ?>
        <?php endforeach ?>
      </div>
        ...
        <?php foreach ($users as $user) : ?>
          <?php if (!is_admin($user)) : ?>
            <div class="button-group">
              <button class="button-action" id="button-ubah" onclick="window.location='/admin/edituser/<?= $user['id'] ?>'">Ubah</button>
    ...
    <div class="page-number-wrapper">
      <div class="page-number-centered">
        ...
        <div class="page-number-container" id="page-numbers">
          <button class="page-number-button" id="button-page-1">1</button>
        </div>
